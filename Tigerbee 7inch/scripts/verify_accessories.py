...
Run after `python -m tigerbee accessories`:
    uv run python scripts/verify_accessories.py [--only ID ...] [--dist DIR] [--skip-freecad]
...
from tigerbee.accessories import build_accessory, discover_accessories, printed, run_checks
...
def main() -> int:
    ...
    for name, mod in mods.items():
        ...
        for n, ok, detail in run_checks(mod, parts):
            check(ok, f"{name}: {n} - {detail}")
